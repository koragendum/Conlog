...
class TokenStream:
    ...
    def __next__(self):
        while True:
            ...
            raise RuntimeError(f"unable to tokenize ({tok_line}, {tok_column})")
...
class TextProgram:
    ...
    def add_node(self, seq):
        """
        Returns (number of tokens consumed, node name) or instance of ParseFailure.
        seq -- nonempty list of Tokens
        """
        if seq[0].kind != 'name':
            return ParseFailure("expected node name", seq[0])

        node_name = seq[0].value
        if node_name in self.variables:
            return ParseFailure("variable with this name already exists", seq[0])
        ...
        if len(seq) < 6:
            return ParseFailure("incomplete node definition", seq)

        if seq[2].kind != 'name':
            return ParseFailure("expected variable name", seq[2])

        if not (seq[3].kind == 'symbol' and seq[3].value in ('+=', '-=')):
            return ParseFailure("expected increment or decrement", seq[3])

        if seq[4].kind not in ('numeric', 'character', 'name'):
            return ParseFailure("expected literal or variable name", seq[5])

        if not (seq[5].kind == 'symbol' and seq[5].value == ']'):
            return ParseFailure("expected a closing bracket", seq[5])

        if seq[2].value in self.nodes:
            return ParseFailure("label refers to an existing node", seq[2])

        if seq[4].kind == 'name' and seq[4].value in self.nodes:
            return ParseFailure("label refers to an existing node", seq[4])

        if node_name in self.nodes and self.nodes[node_name] is not None:
            return ParseFailure("node mutation has already been defined", seq[1:6])
    ...
    def add_statement(self, seq):
        """
        Returns None or instance of ParseFailure.
        seq -- nonempty list of Tokens
        """
        if seq[0].kind != 'name':
            return ParseFailure("statement must begin with a name", seq[0])
        ...
        if seq[1].kind == 'symbol' and seq[1].value == '=':
            if len(seq) < 3:
                return ParseFailure("incomplete variable initialization", seq)
            is_literal = seq[2].kind in ('numeric', 'character')
            is_free    = seq[2].kind == 'symbol' and seq[2].value == '?'
            if not (is_literal or is_free):
                return ParseFailure("variable must be initialized to a constant or marked free", seq[2])
            if len(seq) > 3:
                return ParseFailure("extraneous characters in variable intialization", seq[3:])
            if name in self.nodes:
                return ParseFailure("node with this label already exists", seq[0])
            if name in self.variables:
                if self.variables[name] is not None:
                    return ParseFailure("variable has already been initialized", seq[0])
        ...
        while True:
            ...
            if last_node is not None:
                if this_node == last_node:
                    return ParseFailure("cannot make edge from a node to itself", seq[index])
            ...
            if not (seq[index].kind == 'symbol' and seq[index].value == '--'):
                return ParseFailure("expected edge", seq[index])
            index += 1
